...
<body class="atmo">
  ...
  <div class="modal-backdrop" id="opsModalBackdrop" onclick="handleBackdropClick(event)">
    <div class="ops-modal">
      <div class="modal-header">
        <div class="modal-title">⚡ Graph Operations & Diagnostic Station</div>
        <button class="modal-close-btn" onclick="closeOpsModal()">✕</button>
      </div>
      <div class="modal-nav">
        <button class="modal-nav-btn active" id="modalTabDoctor" onclick="switchOpsTab('doctor')">🔍 Doctor & Metrics</button>
        <button class="modal-nav-btn" id="modalTabRanked" onclick="switchOpsTab('ranked')">⚡ Action Queue & Path</button>
        <button class="modal-nav-btn" id="modalTabExport" onclick="switchOpsTab('export')">💾 Export Artifacts</button>
      </div>
  ...
  <script>
    ...
    // Custom Path Highlight state (e.g. from shortest path finder)
    ...
    function createCardElement(n, upstream) {
      ...
      // Chain highlight & dimming (Recursive upstream only or active custom path)
      let chainClass = '';
      if (activeCustomPath) {
    ...
    function renderDag() {
      ...
      // Compute recursive chains for electrical pulse highlight (upstream only or custom path)
      const upstream = getRecursiveUpstream(selectedNodeId);
      ...
      visible.forEach(n => {
        ...
        // Dimming & selection classes (Recursive upstream or custom path)
        if (activeCustomPath) {
          if (n.id === selectedNodeId) {
    ...
    function switchOpsTab(tab) {
      ['doctor', 'ranked', 'export'].forEach(t => {
      ...
      container.innerHTML = `
        <div class="report-card ${isHealthy ? 'pass' : 'fail'}">
          ...
          <div style="font-size:12px; color:var(--text-secondary);">
            Tarjan Cycles: <strong>${sccs.length}</strong> | Dangling Edges: <strong>${dangling.length}</strong> | Verified Nodes: <strong>${nodes.length}</strong> | Density: <strong>${density}</strong>
          </div>
        </div>
    ...
    function renderActionQueueAndPathTab(container) {
      ...
      container.innerHTML = `
        <div class="report-card">
          <div class="section-label">⚡ Top Ranked Actionable Tasks (Click to Focus on DAG)</div>
        ...
        <div class="report-card">
          <div class="section-label">🧭 Shortest Dependency Path Finder (BFS)</div>
